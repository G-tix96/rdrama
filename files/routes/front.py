...
def frontlist(v=None, sort="hot", page=1, t="all", ids_only=True, ccmode="false", filter_words='', gt=0, lt=0, sub=None, site=None, pins=True):
	...
	if sort == 'hot':
		...
		if SITE_NAME == 'rDrama':
			posts = posts.order_by(-1000000*(Submission.realupvotes + 1 + Submission.comment_count/5 + func.least(100, (func.length(Submission.body_html)-func.length(func.replace(Submission.body_html,'<a href="https://','')))/5 + func.length(Submission.body)/100))/(func.power(((ti - Submission.created_utc)/1000), 1.23)), Submission.created_utc.desc())
		else:
